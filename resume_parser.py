...
def extract_education(text):
    education_pattern = r"(Bachelor|Master|B\.Sc|M\.Sc|PhD|BTech|MTech|MBA|Diploma|BE|B\.E)[\w\s,&]*"
    education = re.findall(education_pattern, text, re.IGNORECASE)
    refined_education = [text[text.lower().find(match.lower()):text.lower().find(match.lower()) + 50].strip() for match in education if text.lower().find(match.lower()) != -1]
    return refined_education if refined_education else ["Not found"]
...
def extract_experience(text):
    experience_pattern = r"(\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s\d{4}\b)(.*?)((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s\d{4}|Present)"
    matches = re.findall(experience_pattern, text, re.IGNORECASE)
    experience = [{"duration": f"{match[0]} - {match[2]}", "description": match[1].strip()} for match in matches]
    return experience if experience else ["Not found"]


def extract_projects(text):
    project_pattern = r"(Project:|Developed|Built|Worked on)[\s\w,:.-]+"
